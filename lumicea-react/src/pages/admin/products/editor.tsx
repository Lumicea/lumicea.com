...
interface Product {
  id: string; name: string; description: string; features: string | null; care_instructions: string | null; processing_times: string | null; base_price: number; slug: string | null; variants: Variant[]; images: string[]; quantity: number | null; is_made_to_order: boolean; is_active: boolean; is_featured: boolean; created_at: string; updated_at: string; categories: string[]; collections: string[]; tags: string[];
}
    ...
    return (
        <div className="space-y-4 rounded-lg border bg-slate-50 p-4">
            ...
            <div className="flex space-x-2">
                ...
                <Button type="button" onClick={handleAdd} className="bg-lumicea-gold text-lumicea-navy hover:bg-lumicea-gold/90">Add New</Button>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {filteredItems.length > 0 ? filteredItems.map(item => (
                        <Badge
                            ...
                            className={`cursor-pointer transition-all text-sm py-1 px-3 ${selectedIds.includes(item.id) ? 'bg-lumicea-navy text-white hover:bg-lumicea-navy/90' : 'bg-white text-lumicea-navy border border-lumicea-navy hover:bg-lumicea-navy/10'}`}
                        >
                            {item.name}
                        </Badge>
    ...
    <div className="bg-gray-50/50 min-h-screen">
      <form onSubmit={handleSubmit}>
        <div className="max-w-5xl mx-auto p-4 md:p-8">
            <div className="flex justify-between items-center mb-8"><div><h1 className="text-3xl font-bold text-gray-900">{!id ? 'Create New Product' : 'Edit Product'}</h1>{id && <p className="text-sm text-gray-500">Editing: {product.name}</p>}</div><div className="flex space-x-2"><Button type="button" variant="outline" onClick={() => navigate('/admin/products')} disabled={isSaving}>Cancel</Button><Button type="submit" disabled={isSaving} className="bg-lumicea-navy text-white hover:bg-lumicea-navy/90">{isSaving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : 'Save Product'}</Button></div></div>
            ...
            <div className="pt-8 flex justify-end space-x-4 border-t mt-8">
                ...
                <Button type="submit" disabled={isSaving} className="bg-lumicea-navy text-white hover:bg-lumicea-navy/90">
