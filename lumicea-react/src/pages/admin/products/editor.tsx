                    ...
                    <CardContent className="space-y-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div className="space-y-2"><Label>Categories</Label>
                                <Popover open={openCategories} onOpenChange={setOpenCategories}><PopoverTrigger asChild><Button variant="outline" className="w-full justify-start font-normal">{product.categories.length > 0 ? `${product.categories.length} selected` : "Select categories..."}</Button></PopoverTrigger>
                                    <PopoverContent className="w-[300px] p-0"><Command><CommandInput placeholder="Search or add new..." value={categoryInput} onValueChange={setCategoryInput} /><CommandList><CommandEmpty><Button variant="link" onMouseDown={(e) => { e.preventDefault(); handleAddNewItem('category'); }}>Add: "{categoryInput}"</Button></CommandEmpty><CommandGroup>{categories.map((cat) => (<CommandItem key={cat.id} onSelect={() => {handleMultiSelectToggle('categories', cat.id)}} onMouseDown={(e) => e.preventDefault()}><Checkbox checked={product.categories.includes(cat.id)} className="mr-2" />{cat.name}</CommandItem>))}</CommandGroup></CommandList></Command></PopoverContent>
                                </Popover>
                            ...
                            <div className="space-y-2"><Label>Collections</Label>
                                <Popover open={openCollections} onOpenChange={setOpenCollections}><PopoverTrigger asChild><Button variant="outline" className="w-full justify-start font-normal">{product.collections.length > 0 ? `${product.collections.length} selected` : "Select collections..."}</Button></PopoverTrigger>
                                    <PopoverContent className="w-[300px] p-0"><Command><CommandInput placeholder="Search or add new..." value={collectionInput} onValueChange={setCollectionInput} /><CommandList><CommandEmpty><Button variant="link" onMouseDown={(e) => { e.preventDefault(); handleAddNewItem('collection'); }}>Add: "{collectionInput}"</Button></CommandEmpty><CommandGroup>{collections.map((col) => (<CommandItem key={col.id} onSelect={() => handleMultiSelectToggle('collections', col.id)} onMouseDown={(e) => e.preventDefault()}><Checkbox checked={product.collections.includes(col.id)} className="mr-2" />{col.collection_name}</CommandItem>))}</CommandGroup></CommandList></Command></PopoverContent>
                                </Popover>
                        ...
                        <div className="space-y-2"><Label>Tags</Label>
                            <Popover open={openTags} onOpenChange={setOpenTags}><PopoverTrigger asChild><Button type="button" variant="outline" className="w-full sm:w-auto">Manage Tags</Button></PopoverTrigger>
                                <PopoverContent className="w-80 p-0"><Command><CommandInput placeholder="Search or add new..." value={tagInput} onValueChange={setTagInput} /><CommandList><CommandEmpty><Button variant="link" onMouseDown={(e) => { e.preventDefault(); handleAddNewItem('tag'); }}>Add: "{tagInput}"</Button></CommandEmpty><CommandGroup>{existingTags.map(tag => (<CommandItem key={tag.id} onSelect={() => handleMultiSelectToggle('tags', tag.id)} onMouseDown={(e) => e.preventDefault()}><Checkbox checked={product.tags.includes(tag.id)} className="mr-2" />{tag.name}</CommandItem>))}</CommandGroup></CommandList></Command></PopoverContent>
                            </Popover>
